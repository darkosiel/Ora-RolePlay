function StartTaxiMission(param)
    local positionStartRandomness = Random(#taxi["startMission"])
    if positionStartRandomness == 0 then
        positionStartRandomness = 1
    end

    local positionStart = taxi["startMission"][positionStartRandomness]
    ...
    ShowAdvancedNotification(
        "Centrale",
        "~b~Dialogue",
        "~g~~h~Des clients attendent un taxi~h~~w~\n\n~o~Va les récupérer !~w~",
        "CHAR_LESTER",
        1
    )
        ...
        while true do
            ...
            if (startDropOff == true) then
                if (currentDropOffPoint == nil) then
                    ...
                    if (currentDropOffIndex <= #taxi.DropOff) then 
                        dropOffPointItems = #taxi.DropOff[currentDropOffIndex]
                        currentDropOffPoint = taxi.DropOff[currentDropOffIndex][math.random(1, dropOffPointItems)]
                        ...
                        local endPosCount = #taxi.endMission
                        endPosition = taxi.endMission[math.random(1, endPosCount)]
