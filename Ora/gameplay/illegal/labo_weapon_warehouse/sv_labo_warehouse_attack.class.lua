              ...
              if (value.STATUS == "running") then
                ...
                for defIndex, defValue in pairs(defenders) do
                    if (LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[defValue.uuid] == nil) then 
                        ...
                        TriggerEvent("Ora::Illegal:sendMessageToOrga", LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[defValue.uuid].organisation_id, "~h~" .. LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[defValue.uuid].fullname .. "~h~ a rejoint le combat.")
                        TriggerEvent("Ora:sendToDiscordFromServer", 999999, 24, "[ATTAQUE #" .. propertyId .. "]\n"..  defValue.fullname .. " a été ajouté dans l'équipe de défense", "info")
                    end
                end
                ...
                for attIndex, attValue in pairs(attackers) do
                    if (LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[attValue.uuid] == nil) then 
                        ...
                        TriggerEvent("Ora::Illegal:sendMessageToOrga", LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[attValue.uuid].organisation_id, "~h~" .. LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[attValue.uuid].fullname .. "~h~ a rejoint le combat.")
                ...
                if (LabsAndWarehouseAttack.ATTACKS[propertyId].SECONDS_LEFT <= 0) then
                  ...
                  LabsAndWarehouseAttack.EndAttack(propertyId)
                    ...
                    IllegalOrga.TriggerClientEvent("Ora::Illegal:updateCurrentAttack", defenderOrganisationId, json.encode(LabsAndWarehouseAttack.ATTACKS[propertyId]))
                    IllegalOrga.TriggerClientEvent("Ora::Illegal:updateCurrentAttack", attackerOrganisationId, json.encode(LabsAndWarehouseAttack.ATTACKS[propertyId]))
                end
              end
          end
      end
...
function LabsAndWarehouseAttack.AnalyzeResults(propertyId)
    if (LabsAndWarehouseAttack.ATTACKS[propertyId] ~= nil) then
        ...
        if (attackObject.FINAL_ALIVE_ATTACKER_IN_ZONE == 0) then
          -- Defender wins
          TriggerEvent("Ora::Illegal:sendMessageToOrga", attackObject.DEFENDER_ORGA_ID, "~g~Félicitations, vous avez sauvez votre propriété.")
          TriggerEvent("Ora::Illegal:sendMessageToOrga", attackObject.ATTACKER_ORGA_ID, "~r~Dommage, vous n'avez pas réussi a capturer. Vous n'avez plus de membres vivants.")
        ...
        elseif (attackObject.FINAL_ALIVE_ATTACKER_IN_ZONE > attackObject.FINAL_ALIVE_DEFENDER_IN_ZONE) then
          -- Attackers wins
          TriggerEvent("Ora::Illegal:sendMessageToOrga", attackObject.DEFENDER_ORGA_ID, "~r~Dommage, vous n'avez pas réussi a défendre (" .. attackObject.FINAL_ALIVE_ATTACKER_IN_ZONE .. " adversaires dans la zone vivants).")
          TriggerEvent("Ora::Illegal:sendMessageToOrga", attackObject.ATTACKER_ORGA_ID, "~g~Félicitations, vous avez capturé la propriété. C'est désormais la votre.")
          TriggerEvent("Ora::Illegal:ChangeOwnerOfProperty", {property_id = propertyId, old_owner = attackObject.DEFENDER_ORGA_ID, new_owner = attackObject.ATTACKER_ORGA_ID})
          TriggerEvent("Ora:sendToDiscordFromServer", 999999, 24, "[ATTAQUE #" .. propertyId .. "]\nles attaquants gagnent (Plus d'attaquants que de defenseurs encore vivant sur place)", "info")
        elseif (attackObject.FINAL_ALIVE_ATTACKER_IN_ZONE == attackObject.FINAL_ALIVE_DEFENDER_IN_ZONE) then
          -- Defender wins
          TriggerEvent("Ora::Illegal:sendMessageToOrga", attackObject.DEFENDER_ORGA_ID, "~g~Félicitations, vous avez sauvez votre propriété.")
          TriggerEvent("Ora::Illegal:sendMessageToOrga", attackObject.ATTACKER_ORGA_ID, "~r~Dommage, vous n'avez pas réussi a capturer (Autant de vivants de chaque côté).")
          TriggerEvent("Ora:sendToDiscordFromServer", 999999, 24, "[ATTAQUE #" .. propertyId .. "]\nles defenseurs gagnent (autant d'attaquants, que de defenseurs vivants sur place)", "info")

        else
          -- Defender wins
          TriggerEvent("Ora::Illegal:sendMessageToOrga", attackObject.DEFENDER_ORGA_ID, "~g~Félicitations, vous avez sauvez votre propriété.")
          TriggerEvent("Ora::Illegal:sendMessageToOrga", attackObject.ATTACKER_ORGA_ID, "~r~Dommage, vous n'avez pas réussi a capturer (" .. attackObject.FINAL_ALIVE_DEFENDER_IN_ZONE .. " adversaires dans la zone vivants).")
          TriggerEvent("Ora:sendToDiscordFromServer", 999999, 24, "[ATTAQUE #" .. propertyId .. "]\nles defenseurs gagnent (moins d'attaquants que de defenseurs vivants sur place)", "info")
        end

        IllegalOrga.TriggerClientEvent("Ora::Illegal:EndAttack", attackObject.DEFENDER_ORGA_ID)
        IllegalOrga.TriggerClientEvent("Ora::Illegal:EndAttack", attackObject.ATTACKER_ORGA_ID)
...
function LabsAndWarehouseAttack.EndAttack(propertyId)
    if (LabsAndWarehouseAttack.ATTACKS[propertyId] ~= nil) then
      ...
      local verificationObjectForAttacker = {
        ...
        serverEventName = "Ora::Illegal:SEAttackerCount",
      }

      local verificationObjectForDefender = {
        warzone = {x = LabsAndWarehouseAttack.ATTACKS[propertyId].PROPERTY.outside_door.x, y = LabsAndWarehouseAttack.ATTACKS[propertyId].PROPERTY.outside_door.y, z = LabsAndWarehouseAttack.ATTACKS[propertyId].PROPERTY.outside_door.z},
        distance = LabsAndWarehouseAttack.MAX_DISTANCE_FIGHT,
        serverEventName = "Ora::Illegal:SEDefenderCount",
      }


      TriggerEvent("Ora::Illegal:sendMessageToOrga", attackerOrganisationId, "~b~L'attaque est terminée. Patientez 15 secondes pendant le calcul de la victoire.")
      TriggerEvent("Ora::Illegal:sendMessageToOrga", defenderOrganisationId, "~b~L'attaque est terminée. Patientez 15 secondes pendant le calcul de la victoire.")
      
      IllegalOrga.TriggerClientEvent("Ora::Illegal:IsInWarzone", attackerOrganisationId, json.encode(verificationObjectForAttacker))
      IllegalOrga.TriggerClientEvent("Ora::Illegal:IsInWarzone", defenderOrganisationId, json.encode(verificationObjectForDefender))
...
RegisterServerEvent("Ora::Illegal:SEDefenderCount")
AddEventHandler("Ora::Illegal:SEDefenderCount",function(resultsAsJson)
...
AddEventHandler("Ora::Illegal:PlayerIsDead",function()
    ...
    if (LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[uuid] ~= nil) then 
        if (LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[uuid].alive == true) then
          ...
          TriggerEvent("Ora::Illegal:sendMessageToOrga", LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[uuid].organisation_id, "~h~" .. LabsAndWarehouseAttack.CURRENT_MONITORED_PLAYERS[uuid].fullname .. "~h~ n'est plus dans le combat.")
...
RegisterServerEvent("Ora::Illegal:StartAttack")
AddEventHandler("Ora::Illegal:StartAttack",function(attackObject)
    local _source = source
    if (LabsAndWarehouseAttack.ATTACKS[attackObject.PROPERTY.id] ~= nil) then
      TriggerClientEvent("Ora::Illegal:ShowNotification", _source, "~r~Une attaque sur la propriété ".. attackObject.PROPERTY.name .." est déjà en cours~s~")
      TriggerEvent("Ora:sendToDiscordFromServer", _source, 24, "[ATTAQUE #" .. attackObject.PROPERTY.id .. "]\n" .. Ora.Identity:GetFullname(_source) .. " tente de lancer une attaque sur " .. attackObject.PROPERTY.name .. " mais une attaque est déjà en cours sur ce batiment", "info")
      return false
    end
    ...
    TriggerEvent("Ora::Illegal:sendMessageToOrga", defenderOrganisationId, "La propriété ~h~" .. attackObject.PROPERTY.name .. "~h~ est attaquée ! Depechez vous de venir sur place.")
    TriggerEvent("Ora::Illegal:sendMessageToOrga", defenderOrganisationId, "Vous avez désormais ".. math.tointeger(timeToHoldInSeconds / 60) .." minutes pour sécuriser la zone.")
    TriggerEvent("Ora::Illegal:sendMessageToOrga", attackerOrganisationId, "Vous attaquez la propriété ~h~" .. attackObject.PROPERTY.name .. "~h~")
    TriggerEvent("Ora::Illegal:sendMessageToOrga", attackerOrganisationId, "Tenez la position pendant ".. math.tointeger(timeToHoldInSeconds / 60) .." minutes.")
    TriggerEvent("Ora::Illegal:setCoordsToOrga", attackerOrganisationId, vector3(attackObject.PROPERTY.outside_door.x, attackObject.PROPERTY.outside_door.y, attackObject.PROPERTY.outside_door.z))
    TriggerEvent("Ora::Illegal:setCoordsToOrga", defenderOrganisationId, vector3(attackObject.PROPERTY.outside_door.x, attackObject.PROPERTY.outside_door.y, attackObject.PROPERTY.outside_door.z))
    ...
    IllegalOrga.TriggerClientEvent("Ora::Illegal:StartAttack", defenderOrganisationId, json.encode(LabsAndWarehouseAttack.ATTACKS[attackObject.PROPERTY.id]))
    IllegalOrga.TriggerClientEvent("Ora::Illegal:StartAttack", attackerOrganisationId, json.encode(LabsAndWarehouseAttack.ATTACKS[attackObject.PROPERTY.id]))
    TriggerEvent("Ora:sendToDiscordFromServer", _source, 24, "[ATTAQUE #" .. attackObject.PROPERTY.id .. "]\n" .. Ora.Identity:GetFullname(_source) .. " vient d'attaquer la propriété " .. attackObject.PROPERTY.name .. ".", "info")
end)