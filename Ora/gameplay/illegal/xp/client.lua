...
function GetXpRequiredNextLvl()
    return math.floor(
        math.floor(XNL_CurrentPlayerXP / levelsRange) + 1 * ((levelsRange * GetPlyLevel()) + 1478 * GetPlyLevel())
    )
end
...
AddEventHandler(
    ...
    function(NetCurrentXP, NetShowRankBar, NetShowRankBarAnimating)
...
function XNL_AddPlayerXP(XPAmount)
    ...
    if LevelDifference > 0 then
        ...
        CreateRankBar(
            XNL_GetXPFloorForLevel(StartAtLevel),
            XNL_GetXPCeilingForLevel(StartAtLevel),
            XNL_CurrentPlayerXP,
            XNL_GetXPCeilingForLevel(StartAtLevel),
            StartAtLevel,
            false
        ...
        CreateRankBar(
            XNL_GetXPFloorForLevel(NewLevel),
            XNL_GetXPCeilingForLevel(NewLevel),
            XNL_CurrentPlayerXP,
            CurrentXPWithAddedXP,
            NewLevel,
            false
...
function XNL_GetLevelFromXP(intXPAmount)
    if is_int(intXPAmount) then
        ...
        if SearchingFor < RockstarRanks[99] then -- Check if the XP amount is smaller than the last item (level 100) in my 'Rockstar XP Requirements level list'
            ...
            CurXPNeeded = 0
            local CurLevelFound = -1
            for i = 1, XNL_MaxPlayerLevel - 99, 1 do -- The - 99 in this loop ensures that the for loop result excludes the first 99 levels which are  -- already 'covered by' the table at the top
                MainAddPerLevel = MainAddPerLevel + 50
                CurXPNeeded = CurXPNeeded + MainAddPerLevel
                CurLevelFound = i
...
function CreateRankBar(
    XP_StartLimit_RankBar,
    XP_EndLimit_RankBar,
    playersPreviousXP,
    playersCurrentXP,
    CurrentPlayerLevel,
