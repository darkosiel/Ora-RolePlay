function doAnim(animName, time, flag, ped, customPos)
    ...
    if
        IsPedRunningRagdollTask(ped) or IsEntityPlayingAnim(ped, animName[1], animName[2], 3) or
            IsPedActiveInScenario(ped) and (not typ or typ < 40)
     then
        ClearPedTasks(ped)
        return
    end
...
function doAnim2(animName, time, flag, ped, customPos)
    if type(animName) ~= "table" then
        animName = {animName}
    end
    ped, flag = ped or PlayerPedId(), flag and tonumber(flag) or false
