...
CheckEntity = function(hit, data, entity, distance)
	local send_options = {}
	local send_distance = {}
	for o, datas in ipairs(data) do
		if M.CheckOptions(datas, entity, distance) then
			local slot = #send_options + 1 
			send_options[slot] = datas
			send_options[slot].entity = entity
			send_distance[datas.distance] = true
		else send_distance[datas.distance] = false end
	end
		...
		while targetActive do
			...
			elseif not hasFocus and IsDisabledControlPressed(0, 24) then
				...
				for k, v in pairs(send_distance) do
					if not (v == false and distance < k) or (v == true and distance > k) then
	...
	if entity then
		...
		for k, v in ipairs(options) do
			...
			if type(v) == "table" then
				table.insert(Entities[entity], v)
			end
		end
	end
end
...
RegisterNetEvent("Qtarget:NetworkAddTargetEntity")
AddEventHandler("Qtarget:NetworkAddTargetEntity", function(netEntityId, parameters)
	AddTargetEntity(NetworkGetEntityFromNetworkId(netEntityId), parameters)
end)
...
RemoveTargetModel = function(models, labels)
	for _, model in pairs(models) do
		...
		for k, v in pairs(labels) do
			if Models[model][k].label == v then
				table.remove(Models[model], k)
			end
		end
	end
end
...
RegisterNetEvent("Qtarget:NetworkRemoveTargetEntity")
AddEventHandler("Qtarget:NetworkRemoveTargetEntity", function(netEntityId)
	RemoveTargetEntity(NetworkGetEntityFromNetworkId(netEntityId))
end)

RemoveLabelFromTargetEntity = function(entity, labels)
	if entity then
		if Entities[entity] then
			for k, v in pairs(Entities[entity]) do
				for _, v2 in pairs(labels) do
					if v.label == v2 then
						table.remove(Entities[entity], k)
					end
				end
			end
		end
...
RegisterNetEvent("Qtarget:NetworkRemoveLabelFromTargetEntity")
AddEventHandler("Qtarget:NetworkRemoveLabelFromTargetEntity", function(netEntityId, labels)
	RemoveLabelFromTargetEntity(NetworkGetEntityFromNetworkId(netEntityId), labels)
end)
...
local RemoveType = function(type, labels)
	for k, v in pairs(labels) do
		if Types[type][k].label == v then
			table.remove(Types[type], k)
		end
	end
end
