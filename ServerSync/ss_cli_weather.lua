...
AddEventHandler(
    "changeWeather",
    function(newWeather, blackout, startup)
    ...
    function()
        while true do
            ...
            if transistioning == false and IsPedInAnyVehicle(PlayerPedId(), false) then
                if
                    string.upper(currentWeather) == string.upper("THUNDER") or
                        string.upper(currentWeather) == string.upper("CLEARING")
                 then
    ...
    function(newWeather, blackout, startup)
        TriggerEvent(
            ...
            "Change the weather.",
            {
                {
                    name = "weatherType",
                    help = "Available types: extrasunny, clear, neutral, smog, overcast, clouds, clearing, rain, thunder, snow, blizzard, snowlight, xmas"
                }
            }
        )
    end
