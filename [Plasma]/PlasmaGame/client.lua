...
local TenuFemme = {
[3] = {model = 3, colorA = 0, colorB = 0},
[4] = {model = 79, colorA = 3, colorB = 5},
[6] = {model = 58, colorA = 3, colorB = 5},

[7] = {model = 0, colorA = 0, colorB = 0},
[8] = {model = 8, colorA = 0, colorB = 0},
[11] = {model = 180, colorA = 3, colorB = 5}

}

local TenuHomme = {
[3] = {model = 4, colorA = 0, colorB = 0},
[4] = {model = 77, colorA = 3, colorB = 5},
[6] = {model = 55, colorA = 3, colorB = 5},

[7] = {model = 0, colorA = 0, colorB = 0},
[8] = {model = 15, colorA = 0, colorB = 0},
[11] = {model = 178, colorA = 3, colorB = 5},
}
                ...
                if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 1.0)then
                    if (incircle == false) then
						if v.typeM == "create" then
							DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour creer votre partie.")
						else
							DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour rejoindre votre partie.")
						end
                    end
                    incircle = true
            ...
            if(Vdist(pos.x, pos.y, pos.z, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z) < 5.0)then
                ...
                if(Vdist(pos.x, pos.y, pos.z, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z) < 1.0)then
                    if (incircleRed == false) then
						-- if v.typeM == "create" then
							DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour quitter la partie.")
						-- else
...
RegisterNUICallback('validate', function(data, cb)
	...
	for theData, value in pairs(data) do
		if theData == "sessionname" then
			if value == "" or value == nil then
				reason = "Nom de partie invalide !"
				break
			end
		elseif theData == "creatorname" then
			if value == "" or value == nil then
				reason = "Vous devez ecrire un pseudo !"
				break
			end
		end
	end
	if reason == "" then
		...
		notification("Partie crée ! Suppréssion au bout de 3 min si non lancée","success")
...
function openMenuMain(data)
	...
	for k,v in pairs(allSession) do
		if v.CurStat == "WaitingPeople" then
			...
			blueitem[k] = UIMenuItem.New("Rejoindre l'équipe bleu ".. tostring(#EquipeBleu) .."/"..tostring(v.nbpersequip), "")
			newitem[k]:AddItem(blueitem[k])
			
			reditem[k] = UIMenuItem.New("Rejoindre l'équipe rouge ".. tostring(#EquipeRouge) .."/"..tostring(v.nbpersequip), "")
			newitem[k]:AddItem(reditem[k])
			
			if GetPlayerFromServerId(k) ~= -1 then
				if GetPlayerPed(GetPlayerFromServerId(k)) == PlayerPedId() then
					launchitem[k] = UIMenuItem.New("Commencer la partie", "Seul le créateur peut lancer la partie")
...
function endTheGame(winner)
	...
	if winner == "tie" then
		message = "~y~égalité"
		message1 = "Pas de chance, égalité"
	else
		if winner == currentTeam then
			message = "~g~Victoire !"
			message1 = "Vous êtes les boss !"
		else
			message = "~r~Defaite !"
			message1 = "Looser !"
		end
...
AddEventHandler('PaintBall:GoToNextMancheMSG', function(winner)
	...
	if winner == "tie" then
		...
		if winner == currentTeam then
			message = "~g~Vous gagnez !"
			message2 = "Continuez comme ça !"
		else
			message = "~r~You loose !"
			message2 = "Wake up ! "
		end
