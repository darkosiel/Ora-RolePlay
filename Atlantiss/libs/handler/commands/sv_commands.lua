...
AddEventHandler("addReportMenuS",function(message, job, orga)
  ...
  TriggerClientEvent("addReportMenu", -1, source, Atlantiss.Identity:GetFullname(source), message, date, job, orga)
end)
...
AddEventHandler("closeReportS",function(index)
  ...
  TriggerEvent("atlantiss:sendToDiscordFromServer", source, webhookreport, Atlantiss.Identity:GetFullname(source) .." a clôturé le ticket n°"..index, "info")
end)

RegisterServerEvent("takeReportS")
AddEventHandler("takeReportS",function(index, id, name, msg, date, status)
  local who = Atlantiss.Identity:GetFullname(source)
  ...
  TriggerEvent("atlantiss:sendToDiscordFromServer", source, webhookreport, Atlantiss.Identity:GetFullname(source) ..pris..index.." de "..name, "info")
end)