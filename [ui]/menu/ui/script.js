$(document).ready(function(){
  ...
  window.addEventListener('message', function(event){
    ...
    if(event.data.menu != undefined && event.data.menu != false){
      this.console.log(event.data.menu)
    }
    if(event.data.menu == 'payment'){
      $(".crosshair").addClass('active');
      $(".menu-payment").addClass('fadeIn');
      idEnt = event.data.idEntity;
      
      // $("#cursorPointer").css("display","none");
    }
    if(event.data.menu == 'pickup'){
      $(".crosshair").addClass('active');
      $(".menu-pickup").addClass('fadeIn');
      idEnt = event.data.idEntity;
  ...
  $('.carJack').on('click', function(e){
    e.preventDefault();
    $.post('http://menu/carJack', JSON.stringify({
      id: idEnt
    }));
  });
  ...
  $('.repair').on('click', function(e){
    e.preventDefault();
    $.post('http://menu/repair', JSON.stringify({
      id: idEnt
    }));
  });

  $('.lavage').on('click', function(e){
    e.preventDefault();
    $.post('http://menu/lavage', JSON.stringify({
      id: idEnt
    }));
  });
  // Functions
  ...
  $('.take').on('click', function(e){
    e.preventDefault();
    $.post('http://menu/take', JSON.stringify({
      id: idEnt
    }));
  });
  ...
  $('.pickup').on('click', function(e){
    e.preventDefault();
    $.post('http://menu/pickup', JSON.stringify({
      id: idEnt
    }));
  });
  $('.byCard').on('click', function(e){
    e.preventDefault();
    $.post('http://menu/byCard', JSON.stringify({
      id: idEnt
    }));
  });

  $('.byLiquide').on('click', function(e){
    e.preventDefault();
    $.post('http://menu/byLiquide', JSON.stringify({
      id: idEnt
    }));
  });
  // Click Crosshair
