...
local function isVehicleBlacklisted(veh)
    for i = 1, #blacklistedVehicle, 1 do
        if blacklistedVehicle[i] == GetEntityModel(veh) then
            return true
        end
    end
    return false
...
Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(50)
            local ped = PlayerPedId()
            if isPedDrivingAVehicle() and not isVehicleBlacklisted(GetVehiclePedIsIn(ped, false)) then
                vehicle = GetVehiclePedIsIn(ped, false)
