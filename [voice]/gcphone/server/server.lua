...
if (AM.Illegal == nil) then
    AM.Illegal = {}
end

if (AM.Illegal.Numbers == nil) then
    AM.Illegal.Numbers = {}
end
...
function getContacts(identifier)
    local result =
        MySQL.Sync.fetchAll(
        "SELECT phone_users_contacts.* FROM phone_users_contacts LEFT JOIN users ON users.identifier = @identifier WHERE phone_users_contacts.identifier = users.phone_number",
        {
            ["@identifier"] = identifier
        }
    )
...
RegisterServerEvent("AM::Illegal:addPhone")
AddEventHandler(
    "AM::Illegal:addPhone",
    function(phoneNumber, callbackEvent)
        AM.Illegal.Numbers[phoneNumber] = callbackEvent
    end
)
...
AddEventHandler(
    ...
    function(source, phone_number, rtcOffer, extraData)
        ...
        if (AM ~= nil and AM.Illegal ~= nil and AM.Illegal.Numbers ~= nil and AM.Illegal.Numbers[phone_number] ~= nil) then
            Wait(math.random(2000, 6000))
            TriggerClientEvent("gcPhone:rejectCall", sourcePlayer)
            TriggerClientEvent(AM.Illegal.Numbers[phone_number], source, phone_number)
        end
    end
)
