...
function setData() {
    ...
    for(let item of param.History) {
        let newItem = `<div class="history-item"><div><p>` + item.src + ` (<span class="remove">-` + item.montant + `$</span>)</p><p>` + item.dest + ` (<span class="add">+` + item.montant + `$</span>)</p></div><div class="history-item-titre"><p>Titre : <span>` + item.title + `</span></p></div><div class="history-item-detail"><p>Détails : <span>` + item.details + `</span></p></div></div>`;
