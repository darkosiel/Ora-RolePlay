...
AddEventHandler('wk:toggleRadar', function()
    ...
    if IsPedSittingInAnyVehicle( ped ) then 
        ...
        if GetVehicleClass( vehicle ) == 18 then
            ...
            Notify("~r~Vous devez être dans une voiture de police.")
        end 
    else 
...
function ToggleSpeedType()
    if radarInfo.speedType == "mph" then 
        ...
        Notify("~b~Le type de vitesse est en Km/h.")
    else 
        radarInfo.speedType = "mph"
        Notify("~b~Le type de vitesse est en MPH.")
    end
end 
...
Citizen.CreateThread(function()
    ...
    while true do 
        ManageVehicleRadar()
        -- Only run 5 times a second, more realistic, also prevents spam 
        Citizen.Wait(200)
    end
end)
